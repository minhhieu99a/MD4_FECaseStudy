function showFormAddMember(id) {
    let content = `<div class="form-outline mb-4">
  <input type="search" class="form-control" id="inputname" onchange="showUserInforByName(${id})">
...
function showUserInforByName(id, page) {
    ...
    $.ajax({
        ...
        type: "GET",
        url: `http://localhost:8080/groupmember/${id}/${name}?page=${page}`,
        success: function (data) {
            let user = data.content;
            let content = '';
            for (let i = 0; i < user.length; i++) {
                if (user[i].groupId == id) {
                    content += `<li>
                            <div class="nearly-pepls">
                                <figure>
                                    <img src="${'http://localhost:8080/image/' + user[i].avatar}" alt="">
                                </figure>
                                <div class="pepl-info">
                                    <h3>${user[i].fullName}</h3>               
                                </div>
                            </div>
                           </li>`
                }
                ...
                for (let j = i + 1; j < user.length; j++) {
                    if (user[i].userInfoId == user[j].userInfoId && user[j].groupId==id) {
                        break;
                    }else {
                        content += `<li>
                            <div class="nearly-pepls">
                                ...
                                <div class="pepl-info">
                                    <h3>${user[i].fullName}</h3>               
                                    <button style="background-color: #0a98dc ;margin-left:100% ;margin-bottom: 20px" onclick="addMember(${user[i].userinfoId},${id})" >Add member</button>
                                </div>
                            </div>
                          </li>`
                    }}
            ...
            $("#datatable1").html(content);

            let iconPage = `<button id="first" onclick="showUserInforByName(0)"><i class="fa-solid fa-backward-fast">First</i></button> 
                <button  id="backup" onclick="showUserInforByName(${data.pageable.pageNumber} - 1)"><i class ="fa-solid fa-backward-step">Back</i></button>
                      <span> Trang </span> <span>${data.pageable.pageNumber + 1}/ ${data.totalPages}</span>
                      <button id="next" onclick="showUserInforByName(${data.pageable.pageNumber}+1)" ><i class="fa-solid fa-forward-step">Next</i></button>
                        <button id="last" onclick="showUserInforByName(${data.totalPages} -1)"><i class="fa-solid fa-forward-fast">Last</i></button>`
...
function addMember(userId, groupId) {
    $.ajax({
        type: "POST",
        url: `http://localhost:8080/groupmember/${groupId}/${userId}`,
        success: function () {
            showUserInforByName(groupId, page)
        }
    })
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
                content += `<li>
                                <div class="nearly-pepls">
                                    <figure>
                                        <a  title=""><img src="${'http://localhost:8080/image/' + data[i].backGroundUrl}" ></a>
                                    </figure>
                                    <div class="pepl-info">
                                    <h4>(${data[i].name})</h4>
<!--                                        <h4><a onclick="showPageDetail(${data[i].id})" title="(${data[i].groupName1})">(${data[i].groupName1})</a></h4>-->
<!--                                        <a href="#" title="" class="add-butn" data-ripple="">leave group</a>-->
                                        <button type="button" onclick="showPageDetail(${data[i].id})" style="background-color: #0a98dc ;margin-left:80% ;margin-bottom: 20px" >Join Group</button>
                                    </div>
                                </div>
                            </li>`
            }
...
function showAllMember(id, page) {
    $.ajax({
        type: "GET",
        url: `http://localhost:8080/groupmember/${id}?page=${page}`,
        success: function (data) {
            ...
            for (let i = 0; i < members.length; i++) {
                if (members[i].status) {
                    content += `<li>
                            <div class="nearly-pepls">
                                <figure>
                                    <img src="${'http://localhost:8080/image/' + members[i].userInfo.avatar}" alt="">
                                </figure>
                                <div class="pepl-info">
                                    <h3>${members[i].userInfo.fullName}</h3>            
                                    <p>Admin Group</p>                  
                                    <button style="background-color: #0a98dc ;margin-left:100% ;margin-bottom: 20px" onclick="deleteMember(${members[i].id},${members[i].group1.id})" >Delete</button>
                                    <button style="background-color: #0a98dc ;margin-left:100% ;margin-bottom: 20px" onclick="editStatus(${members[i].id},${members[i].group1.id})" >Edit Status </button>
                                </div>
                            </div>
                          </li>`
                } else {
            ...
            let iconPage = `<button id="first" onclick="showAllMember(0)"><i class="fa-solid fa-backward-fast">First</i></button> 
                <button  id="backup" onclick="showAllMember(${data.pageable.pageNumber} - 1)"><i class ="fa-solid fa-backward-step">Back</i></button>
                      <span> Trang </span> <span>${data.pageable.pageNumber + 1}/ ${data.totalPages}</span>
                      <button id="next" onclick="showAllMember(${data.pageable.pageNumber}+1)" ><i class="fa-solid fa-forward-step">Next</i></button>
                        <button id="last" onclick="showAllMember(${data.totalPages} -1)"><i class="fa-solid fa-forward-fast">Last</i></button>`
            $(`#iconPage`).html(iconPage);
            if (data.pageable.pageNumber === 0) {
                document.getElementById("backup").hidden = true
                document.getElementById("first").hidden = true
...
function deleteMember(id, groupId, page) {
    $.ajax({
        type: "DELETE",
        url: `http://localhost:8080/groupmember/${groupId}/${id}?page=${page}`,
        success: function () {
            showAllMember(groupId, page)
        }
    })
    event.preventDefault();
}
...
function editbgr(id) {
    let image = $(`#imageGroup`);
    let editform = new FormData;
    editform.append('backGroundUrl', image.prop('files')[0])
    $.ajax({
        type: "POST",
        ...
        contentType: false,
        data: editform,
        url: `http://localhost:8080/group1/${id}/avt`,
        success: function (data) {
            showPageDetail(data.id)
        }
    })
    event.preventDefault()
}
...
function showPageDetail(id) {
    $.ajax({
        ...
        success: function (data) {
            let content = `<div class="se-pre-con"></div>
...
<a class="fa fa-sliders" href="#shoppingbag"></a>
    ...
    <section>
        <div class="feature-photo">
            <figure><img src="http://localhost:8080/image/${data.backGroundUrl}" alt=""></figure>
            <!--\t\t\tBắt đầu với thêm mới thành viên -->
            <!--\t\t\tBắt đầu với thêm mới thành viên -->
            <!--\t\t\tBắt đầu với thêm mới thành viên -->
            <div class="add-btn">
                <span>1205 followers</span>
                <!--\t\t\t\t<button type="button" class="btn btn-primary" onclick="showFormAddMember()">Add Member</button>-->
                <button type="button" class="btn btn-primary" onclick="showFormAddMember(${data.id})" data-bs-toggle="modal"
                        data-bs-target="#myModal">Add New Member
                </button>
            </div>
            <form class="edit-phto">
                ...
                <label class="fileContainer">
                    Edit Cover Photo
                    <input type="file" id="imageGroup" onchange="editbgr(${data.id})"/>
                </label>
            </form>
                        ...
                        <div class="timeline-info">
                            <ul>
                                ...
                                <li>
                                    ...
                                    <a class="" onclick="showAllMember(${data.id})" data-ripple="">Members</a>
...
<ins>200</ins>
                                ...
                                <div style="text-align: center" id="iconPage"></div>
                            </div><!-- centerl meta -->
                            <div class="col-lg-3">
                                <aside class="sidebar static">
                                    <div class="widget">
